...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void getCurrentLocation() async{
    await Permission.location.request().then((value){
      if (value==PermissionStatus.granted) {
        //거리가 10미터 이상 변해야 리스너에 위치 전달
        Geolocator.getPositionStream(locationSettings: locationSettings).listen((Position position)=> newPosition(position));
      }
    });
  ...
  void initState(){
    ...
    setCustomMarker().then((value) => getCurrentLocation());
  }
  ...
  callSnackBar(msg){
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        // shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10),side: BorderSide(color: Colors.red,width: 2)),
      )
    );
  }
