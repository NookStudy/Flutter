...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void getJsonData(){ //json 파싱
    //안쪽 데이터는 더블쿼네이션으로 해야 에러가 안난다. 더미데이터 쓸때나 이렇게 박음
    String json = '''
{"items":
          {"item":
            [
...
''';
    ...
    var data1 = jsonDecode(json); //최초 파싱
    print(data1);


    //파싱을 원하는 keyt값은 배열기호를 통해 접근한다.
    List data2 = data1['items']['item'];
    print(data2);

    print('================================================');
    for (var item in data2) {
      //name만 가져오거나 id만 가져오거나 가능
      print(item['name']);
    }
  }
...
''';
    ...
    xml2json.parse(xmlData);          //xml파싱
    var json = xml2json.toParker();   //형식옵션
